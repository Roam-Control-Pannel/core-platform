...
export function OfferCard({ offer, showVenue = false }: { offer: ConsumerOffer; showVenue?: boolean }) {
  ...
  const toggleSave = async () => {
    if (!signedIn) { setNote(t("signInToSave")); return; }
    const next = !saved;
    setSaved(next);
  ...
  const redeem = async () => {
    if (!signedIn) { setNote(t("signInToRedeem")); return; }
    setBusy(true);
    setNote(null);
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNote(res.reason === "sold_out" ? t("soldOut") : t("expired"));
      }
    } catch {
      setNote(t("redeemFailed"));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div style={{ padding: "var(--space-3) var(--space-4)", borderRadius: "var(--r-lg)", background: "var(--crimson-tint)", border: "1px solid var(--crimson-tint-2)" }}>
      ...
      {offer.endsAt ? <div style={{ marginTop: 4, fontSize: 11.5, color: "var(--muted)" }}>{t("ends", { date: shortDate(offer.endsAt) })}</div> : null}
      ...
      {redeemed ? (
        <div style={{ marginTop: "var(--space-3)", display: "flex", alignItems: "center", gap: "var(--space-2)", flexWrap: "wrap" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 13, fontWeight: 600, color: "var(--success)" }}>{t("redeemed")} <Icon name="check" size={14} /></span>
          {code ? (
            ...
            <span style={{ fontSize: 12.5, color: "var(--ink-2)" }}>{t("showToStaff")}</span>
          )}
        </div>
      ) : (
        <div style={{ marginTop: "var(--space-3)", display: "flex", alignItems: "center", gap: "var(--space-2)", flexWrap: "wrap" }}>
